...
public class MeansBuilder {
    ...
    private void writeNDArrayToFile(INDArray[] array) {
        try {
            DataBuffer dataBuffer = array[0].data();
            Files.write(Paths.get("./output.txt"), (dataBuffer.toString()).getBytes(), StandardOpenOption.CREATE, StandardOpenOption.APPEND);
        } catch (IOException e) {
            e.printStackTrace();
        }
